...
varae = vae.VAE(cm_data.shape[1:], hyper_dim)
varae.model.load_weights(model_weight)

cm_pred = np.array([varae.decode(np.expand_dims(sample,axis=0)) for sample in cm_data[:npred]])
cm_emb = np.array([varae.return_embeddings(np.expand_dims(sample,axis=0)) for sample in cm_data])
